...
void ads1_data_callback(float * sample);
void ads1_hal_delay(uint16_t delay_ms);
void ads1_hal_pin_int_enable(bool enable);
static void ads2_service_irq_once();
static void ads1_service_irq_once();
...
typedef void (*ads1_callback)(float*);
/* Command set for ADS */
...
{
	ADS1_DEV_UNKNOWN     = 0,
	ADS1_DEV_ONE_AXIS_V1 = 1,
	ADS1_DEV_TWO_AXIS_V1 = 2,
	ADS1_DEV_ONE_AXIS_V2 = 12,
	ADS1_DEV_TWO_AXIS_V2 = 22
} ADS1_DEV_TYPE_T;
...
typedef enum 
{
	ADS1_CALIBRATE_FIRST = 0,		   	// First calibration point, typically 0 degrees
	ADS1_CALIBRATE_SECOND,			     	// Second calibration point, 45-255 degrees
	ADS1_CALIBRATE_CLEAR,				    // Clears user calibration, restores factory calibration
//	ADS1_CALIBRATE_STRETCH_ZERO,			// 0mm strain calibration point
//	ADS1_CALIBRATE_STRETCH_SECOND,		// Second calibration point for stretch, typically 30mm
} ADS1_CALIBRATION_STEP_T;
...
{
  ADS1_SPS_T sps1;
	ads1_callback ads1_sample_callback;
	uint32_t reset_pin;
	uint32_t ADS1_datardy_pin;
  uint8_t  addr2;      // kannna
} ads1_init_t;
...
static uint8_t read_buffer_2[ADS1_TRANSFER_SIZE];
static volatile bool ads1_irq_pending = false;
static volatile uint32_t ads1_lastTickISR = 0;  // tiny debounce window

static inline void ads1_mask_irq_from_isr() { gpio_intr_disable((gpio_num_t)ADS1_INTERRUPT_PIN); }
static inline void ads1_unmask_irq()        { gpio_intr_enable((gpio_num_t)ADS1_INTERRUPT_PIN);  }


volatile bool _ads2_int_enabled = false;
volatile bool _ads1_int_enabled = false;
...
static uint8_t _address1 = ADS1_DEFAULT_ADDR;   // 0x13    Two Axis
int Kan_test = 0;
...
static uint8_t ads1_addrs[ADS_COUNT] = 
{
	ADS1_DEFAULT_ADDR,
};
...
volatile bool ads1_newData = false;

// holds the app’s sample callback
static ads2_callback ads2_sample_callback = nullptr;
static void (*ads2_read_callback)(uint8_t *);

static ads1_callback ads1_sample_callback = nullptr;
static void (*ads1_read_callback)(uint8_t *);
...
int ads1_hal_write_buffer(uint8_t * buffer, uint8_t len);
void ads2_hal_set_address(uint8_t address);
void ads1_hal_set_address(uint8_t address);
void ads2_hal_reset(void);
void ads1_hal_reset(void);
// Two axis 
...
void IRAM_ATTR ads1_hal_interrupt(void)
{
  // One-shot mask to avoid re-entrancy / bounce storms
  ads1_mask_irq_from_isr();

  // Optional debounce (~2 ms). Use 0–5 ms depending on your source.
 uint32_t now = xTaskGetTickCountFromISR();
 if ((now - ads1_lastTickISR) >= pdMS_TO_TICKS(2)) 
	{
    ads1_lastTickISR = now;
    ads1_irq_pending = true;
    //Serial.println("INTERRUPT");
    //Serial.println("IRQ RECEIVED");  			
		 // Do the I2C read in loop()
  } 
...
static void ads1_service_irq_once() 
{
 if (ads1_hal_read_buffer(read_buffer_2, ADS1_TRANSFER_SIZE) == ADS_OK)
 {
    if (ads1_read_callback) 
    ads1_read_callback(read_buffer_2);
 }
 
 ads1_unmask_irq();  // re-arm the GPIO interrupt
}
...
int ads2_hal_init(void (*callback)(uint8_t*), uint32_t reset_pin, uint32_t ADS2_datardy_pin)
{
   ...
   pinMode(ADS2_INTERRUPT_PIN, INPUT_PULLUP);
	...
	I2C_2.begin(I2C1_SDA,I2C1_SCL,400000); // Pin 8 = I2C_SDA , Pin 9 = I2C_SCL
	//Wire.setClock(100000);  
	I2C_2.setTimeOut(50);       // ms
...
int ads1_hal_init(void (*callback)(uint8_t*), uint32_t reset_pin, uint32_t ADS1_datardy_pin)
{
	 pinMode(ADS1_RESET_PIN, OUTPUT);
   pinMode(ADS1_INTERRUPT_PIN, INPUT_PULLUP);
	// Set callback pointer
	ads1_read_callback = callback;
	...
	//Wire.setPins(sdaPin, sclPin); // Replace sdaPin and sclPin with your chosen GPIOs // kannana
	//Wire.setPins(18, 17); // Replace sdaPin and sclPin with your chosen GPIOs   // kannan Nitto
	I2C_1.begin(I2C2_SDA,I2C2_SCL,400000); // Pin 8 = I2C_SDA , Pin 9 = I2C_SCL
	//Wire.setClock(100000);
	I2C_1.setTimeOut(50);          // ms
// Configure and enable interrupt pin
	ads1_hal_pin_int_init();
...
{
  // Low pass IIR filter
  signal_filter1(sample);

   //Serial.println("ads2_data_callback kan");		
  // Deadzone filter
  deadzone_filter1(sample);
...
void ads1_two_axis_parse_read_buffer(uint8_t * buffer)
{
	if(buffer[0] == ADS2_SAMPLE)
	{
		float sample[2];
   ...
   //Serial.println("ads_two_axis_parse_read_buffer");		
    ...
    if (ads1_sample_callback) ads1_sample_callback(sample);  // call user sample callback  
...
int ads1_two_axis_run(bool run)
{
	uint8_t buffer[ADS2_TRANSFER_SIZE];
	...
	return ads1_hal_write_buffer(buffer, ADS1_TRANSFER_SIZE);
}
...
int ads1_two_axis_set_sample_rate(ADS1_SPS_T sps1)
{
	uint8_t buffer[ADS1_TRANSFER_SIZE];
	
	buffer[0] = ADS1_SPS;
	ads_uint16_encode(sps1, &buffer[1]);
...
int ads1_two_axis_enable_interrupt(bool enable)
{
	uint8_t buffer[ADS1_TRANSFER_SIZE];
	
	buffer[0] = ADS1_INTERRUPT_ENABLE;
	buffer[1] = enable;
...
int ads1_two_axis_update_device_address(uint8_t device, uint8_t address)
{
	uint8_t buffer[ADS1_TRANSFER_SIZE];
	
	buffer[0] = ADS1_SET_ADDRESS;
	buffer[1] = address;
	
	if(ads1_hal_write_buffer(buffer, ADS1_TRANSFER_SIZE) != ADS_OK)
		return ADS_ERR_IO;
	
	ads1_hal_set_address(address);
...
int ads1_two_axis_init(ads1_init_t *ads1_init)
{	
	ads1_hal_init(&ads1_two_axis_parse_read_buffer, ads1_init->reset_pin, ads1_init->ADS1_datardy_pin);	// ads_read_callback was set in ads2_hal_init() to point to ads_two_axis_parse_read_buffer().
	                                                                                            // ads_two_axis_parse_read_buffer() is called with the raw I2C data.

  ads1_sample_callback = ads1_init->ads1_sample_callback;     
	// Check that the device id matched ADS_TWO_AXIS
	// Check that the device type is a one axis
	ADS1_DEV_TYPE_T ads1_dev_type;
	if (ads1_get_dev_type(&ads1_dev_type) != ADS_OK)
		return ADS_ERR_DEV_ID;

	switch (ads1_dev_type)
	{
	case ADS1_DEV_TWO_AXIS_V1:
	case ADS1_DEV_TWO_AXIS_V2:
		break;
	default:
		return ADS_ERR_DEV_ID;
	}
	...
	if(ads1_two_axis_set_sample_rate(ads1_init->sps1))     // Set sample rate 
		return ADS_ERR;
...
int ads1_two_axis_enable_axis(uint8_t axes_enable)
{
	if(!(axes_enable & (ADS_AXIS_0_EN | ADS_AXIS_1_EN)))
			return ADS_ERR_BAD_PARAM;
...
int ads1_get_dev_id(void)
{
	ADS1_DEV_TYPE_T device_type;
	
	if (ads1_get_dev_type(&device_type) == ADS_OK)
	{
		switch (device_type)
		{
		case ADS1_DEV_TWO_AXIS_V1:
...
int ads1_get_dev_type(ADS1_DEV_TYPE_T * ads1_dev_type)
{
	uint8_t buffer[ADS1_TRANSFER_SIZE];
	
	buffer[0] = ADS1_GET_DEV_ID;
	
	// Disable interrupt to prevent callback from reading out device id
	ads1_hal_pin_int_enable(false);
	
	ads1_hal_write_buffer(buffer, ADS1_TRANSFER_SIZE);
	ads_hal_delay(2);
	ads1_hal_read_buffer(buffer, ADS1_TRANSFER_SIZE);
	
	ads1_hal_pin_int_enable(true);
	
	if (buffer[0] == ADS1_DEV_ID)
	{
		switch (buffer[1])
		{
		case ADS1_DEV_ONE_AXIS_V1:
		case ADS1_DEV_ONE_AXIS_V2:
		case ADS1_DEV_TWO_AXIS_V1:
		case ADS1_DEV_TWO_AXIS_V2:
			*ads1_dev_type = static_cast<ADS1_DEV_TYPE_T>(buffer[1]);
			return ADS_OK;
		}
	}
	
	*ads1_dev_type = ADS1_DEV_UNKNOWN;
	return ADS_ERR_DEV_ID;
}
...
static void ads1_hal_pin_int_init(void)
{
	pinMode(ADS1_INTERRUPT_PIN, INPUT_PULLUP);
	ads1_hal_pin_int_enable(true);
}
...
void ads1_hal_pin_int_enable(bool enable)
{
	_ads1_int_enabled = enable;
	
	if(enable)
	{
		attachInterrupt(digitalPinToInterrupt(ADS1_INTERRUPT_PIN), ads1_hal_interrupt, FALLING);
	}
	else
	{
		detachInterrupt(digitalPinToInterrupt(ADS1_INTERRUPT_PIN));
	}
}
...
int ads2_hal_write_buffer(uint8_t * buffer, uint8_t len)
{
	...
	I2C_2.beginTransmission(0x13);
	uint8_t nb_written = I2C_2.write(buffer, len);
...
{
	// Disable the interrupt
	if(_ads1_int_enabled)
		detachInterrupt(digitalPinToInterrupt(ADS1_INTERRUPT_PIN));
	
	I2C_1.beginTransmission(_address1);
	...
	if(_ads1_int_enabled)
	{
		...
		// Read data packet if interrupt was missed
		if(digitalRead(ADS1_INTERRUPT_PIN) == 0)
		{
			if(ads1_hal_read_buffer(read_buffer_2, ADS1_TRANSFER_SIZE) == ADS_OK)
			{
				ads1_read_callback(read_buffer_2);
...
int ads2_hal_read_buffer(uint8_t * buffer, uint8_t len)
{
	I2C_2.requestFrom(_address2, len);
...
int ads1_hal_read_buffer(uint8_t * buffer, uint8_t len)
{
	I2C_1.requestFrom(_address1, len);
...
void ads1_hal_reset(void)
{
	...
	pinMode(ADS1_RESET_PIN, OUTPUT);
  ads1_hal_gpio_pin_write(ADS1_RESET_PIN, 0);
	ads_hal_delay(10);
  ads1_hal_gpio_pin_write(ADS1_RESET_PIN, 1);
  pinMode(ADS1_RESET_PIN, INPUT_PULLUP);
}
void ads2_hal_reset(void)
{
	// Configure reset line as an output
...
int ads1_hal_select_device(uint8_t device)
{
	if(device < ADS_COUNT)
		_address1 = ads1_addrs[device];
	else
		return ADS_ERR_BAD_PARAM;
...
int ads1_hal_update_device_addr(uint8_t device, uint8_t address)
{
	if(device < ADS_COUNT)
		ads1_addrs[device] = address;
	else
		return ADS_ERR_BAD_PARAM;
...
uint8_t ads1_hal_get_address(void)
{
	return _address1;
}
...
int ads2_calibrate(ADS2_CALIBRATION_STEP_T ads2_calibration_step, uint8_t degrees)
 ...
 int ads1_calibrate(ADS1_CALIBRATION_STEP_T ads1_calibration_step, uint8_t degrees)
 {
	uint8_t buffer[ADS1_TRANSFER_SIZE] = {0};
	
	buffer[0] = ADS1_CALIBRATE;
	buffer[1] = ads1_calibration_step;
	buffer[2] = degrees;	
	return ads1_hal_write_buffer(buffer, ADS1_TRANSFER_SIZE);
 }
...
void setup()
 ...
 {  
  ...
  if(ret_val == ADS_OK)
  {
    Serial.println("Two Axis ADS1 initialization succeeded");
  }
  else
  {
    Serial.print("Two Axis ADS1 initialization failed with reason: ");
    Serial.println(ret_val);		
  }

  init1.sps1                  = ADS1_10_HZ;  
  init1.ads1_sample_callback  = &ads1_data_callback;   // ads1_sample_callback is set to point to your function ads2_data_callback.
	init1.reset_pin             = ADS1_RESET_PIN;
  init1.ADS1_datardy_pin      = ADS1_INTERRUPT_PIN;
	//init.addr2                  = 0x13;            // Initialize ADS hardware abstraction layer, and set the sample rate

	int ret_val_1 = ads1_two_axis_init(&init1);       //   ads2_two_axis_init       
...
ads1_hal_set_address(ADS1_DEFAULT_ADDR);
ads1_two_axis_run(true);
delay(1200);
...
//   } 
 ...
 {
  noInterrupts(); 
	ads1_irq_pending = false; 
	interrupts();
  ads1_service_irq_once();
 }
 
if(ads1_newData && ads2_newData)
 {
   ads1_newData = false;
	 ads2_newData = false;        
  //  Horiz_Angle_2 = ang[0]; 
